...
function openSinglePlaceMap(lat, lng, name) {
    closeModal();
    // ถ้าอยู่หน้า ai.html ให้เลื่อนไปที่แผนที่ AI หรือเปิด modal แผนที่พิเศษ
    const aiMapSec = document.getElementById('aiRouteMap');
    if (aiMapSec) {
        aiMapSec.scrollIntoView({ behavior: 'smooth', block: 'center' });
        const singleStop = [{ lat: lat, lng: lng, title: name, spot: name, time: "จุดหมาย" }];
        renderAIItineraryMap(singleStop);
        showToast(`โหลดแผนที่นำทางไปยัง "${name}" แล้ว`);
    } else {
        // พาไปหน้า ai.html พร้อมพารามิเตอร์หรือเปิดโหมดแผนที่เดี่ยว
        window.location.href = `ai.html?lat=${lat}&lng=${lng}&name=${encodeURIComponent(name)}`;
    }
}
