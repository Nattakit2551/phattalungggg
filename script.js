...
// เปิดแผนที่นำทางเต็มจอสำหรับทริป AI หลายจุด
function openFullAiMapNavigation() {
    if (!currentPlannedStops || currentPlannedStops.length === 0) {
        showToast('กรุณากดสร้างแผนการเดินทางด้วย AI ก่อน', 'error');
        return;
    }
    
    // แปลง stops เป็น JSON เพื่อส่งผ่าน URL หรือ localStorage
...
async function initDedicatedNavigationPage() {
    ...
    if (mode === 'trip') {
        // โหมดทริปหลายจุดจาก AI
        const tripDataJson = localStorage.getItem('phatthalung_ai_trip');
        if (tripDataJson) {
            const stops = JSON.parse(tripDataJson);
            document.getElementById('navPageTitle').innerText = `แผนที่นำทางทริป AI (${stops.length} จุดหมาย)`;
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(pos => {
                    ...
                    routingStops.unshift({ lat: uLat, lng: uLng, title: "ตำแหน่งที่คุณอยู่" });
            ...
            stops.forEach((st, idx) => {
                L.marker([st.lat, st.lng]).addTo(dedicatedMap)
                    .bindPopup(`<b>จุดที่ ${idx + 1}: ${st.title}</b><br>${st.spot}`);
            });
            ...
            try {
                ...
                if (data.routes && data.routes.length > 0) {
                    ...
                    document.getElementById('mobileHudInstruction').innerText = `เส้นทางทริป AI รวม ${stops.length} จุด (${km} กม.)`;
        ...
        // โหมดนำทางจุดเดี่ยว (Single Place)
        const lat = urlParams.get('lat');
        const lng = urlParams.get('lng');
        const name = urlParams.get('name');
