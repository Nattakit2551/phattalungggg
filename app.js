...
$$("[data-close]").forEach(el=>el.onclick=e=>{
  ...
  const mv = document.getElementById("modalImgVideo");
  if(mv){ mv.pause(); mv.classList.remove("show"); }
});
...
$$(".auth-tabs button").forEach(b=>b.onclick=()=>{
  ...
  /* เปลี่ยนข้อความต้อนรับตามแท็บ */
  const w = $("#authWelcome");
  if(w){
    if(t==="login") w.innerHTML = "<h3>ยินดีต้อนรับกลับมา 👋</h3><p>เข้าสู่ระบบเพื่อบันทึกที่เที่ยวโปรดของคุณ</p>";
    else w.innerHTML = "<h3>มาเป็นสมาชิกกัน ✨</h3><p>สมัครฟรี เก็บที่เที่ยวโปรดและวางแผนทริปได้</p>";
    w.classList.remove("wel-anim"); void w.offsetWidth; w.classList.add("wel-anim");
  }
});
...
/* ═══ เลือกสื่อของสถานที่: วิดีโอเฉพาะ → วิดีโอหมวด → ภาพนิ่งซูมช้า ═══
   วางไฟล์วิดีโอ (ถ้ามี) ในโฟลเดอร์ videos/ :
   - เฉพาะที่:  videos/place-<id>.mp4   เช่น videos/place-1.mp4
   - ตามหมวด:  videos/cat-<หมวด>.mp4   เช่น videos/cat-ธรรมชาติ.mp4
   ถ้าไม่มีไฟล์ ระบบจะใช้ภาพนิ่งซูมช้า (Ken Burns) อัตโนมัติ */
function placeVideoSources(p){
  return [
    `videos/place-${p.id}.mp4`,
    `videos/cat-${p.cat}.mp4`,
  ];
}
function applyPlaceMedia(p, imgEl, videoEl){
  if(!imgEl) return;
  /* ค่าเริ่มต้น: ภาพนิ่งซูมช้า */
  ...
  /* ลองโหลดวิดีโอทีละแหล่ง ถ้าเล่นได้ค่อยโชว์ทับภาพ */
  const sources = placeVideoSources(p);
  let idx = 0;
  const tryNext = ()=>{
    if(idx >= sources.length){ return; }  // ไม่มีวิดีโอ → คงภาพนิ่งซูม
    const url = sources[idx++];
    videoEl.src = url;
    videoEl.load();
    const onOk = ()=>{ videoEl.play().then(()=>{
...
window.applyPlaceMedia = applyPlaceMedia;
...
function openPlace(id){
  ...
  applyPlaceMedia(p, mi, $("#modalImgVideo"));
  ...
  if($("#modalFull")) $("#modalFull").href = `place.html?id=${p.id}`;
  ...
  /* พูดอัตโนมัติทันทีที่เปิด */
  setTimeout(()=>narratePlace(), 350);
}
...
function renderPlaces(){
  ...
  loadImgs();
  window.dispatchEvent(new CustomEvent("ptl-places-updated"));
}
if($("#searchInput")) $("#searchInput").oninput = e=>{state.search=e.target.value;renderPlaces()};

/* ═══ แผนที่รวมสถานที่ (Leaflet + OpenStreetMap) ═══ */
(function(){
  const mapEl = $("#placesMap");
  const toggle = $("#viewToggle");
  if(!mapEl || !toggle || typeof L === "undefined") return;
  ...
  function makeIcon(cat){
    const color = CAT_COLOR[cat] || "#106a75";
    return L.divIcon({
      className:"pin-wrap",
  ...
  function initMap(){
    if(map) return;
    map = L.map(mapEl,{scrollWheelZoom:false}).setView([7.55, 100.05], 10);
    L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{
      attribution:'© OpenStreetMap', maxZoom:18
    }).addTo(map);
    /* คลิกที่แผนที่ให้ซูมได้เมื่อโฟกัส */
  ...
  function refreshMarkers(){
    ...
    list.forEach(p=>{
      ...
      const m = L.marker([p.lat,p.lng],{icon:makeIcon(p.cat)}).addTo(map);
      m.bindPopup(
        `<div class="map-pop">
          <b>${p.name}</b>
          <span class="map-pop-cat" style="color:${CAT_COLOR[p.cat]||'#106a75'}">${p.cat}</span>
          <span class="map-pop-addr">${p.addr}</span>
          <button class="map-pop-btn" data-id="${p.id}">ดูรายละเอียด</button>
        </div>`
      );
      markers.push(m);
    ...
    /* ปุ่มในป๊อปอัพเปิด modal */
    map.on("popupopen", e=>{
      const btn = e.popup.getElement().querySelector(".map-pop-btn");
      if(btn) btn.onclick = ()=>openPlace(+btn.dataset.id);
    });
  }

  /* สลับมุมมอง การ์ด ⇄ แผนที่ */
  toggle.querySelectorAll(".vt-btn").forEach(b=>b.onclick=()=>{
    toggle.querySelectorAll(".vt-btn").forEach(x=>x.classList.remove("active"));
    b.classList.add("active");
    const isMap = b.dataset.view==="map";
    $("#placesGrid").classList.toggle("hidden", isMap);
    mapEl.classList.toggle("hidden", !isMap);
    if(isMap){
      initMap();
      setTimeout(()=>{ map.invalidateSize(); refreshMarkers(); }, 60);
    }
  });

  /* อัปเดตหมุดเมื่อกรอง/ค้นหา ถ้ากำลังดูแผนที่อยู่ */
  const _renderPlaces = window.renderPlaces;
  window.addEventListener("ptl-places-updated", ()=>{
    if(map && !mapEl.classList.contains("hidden")) refreshMarkers();
  });
})();
renderChips();
renderPlaces();
...
/* วิดีโอพื้นหลัง (หน้าแรก + หน้ารอง) — เฟดเข้าเมื่อเล่นได้ / ถ้าเล่นไม่ได้ใช้ภาพนิ่งแทน */
document.querySelectorAll("#heroVideo, #pageHeroVideo").forEach(v=>{
  const show = ()=> v.classList.add("playing");
  const hide = ()=> v.classList.remove("playing");   // เผยภาพนิ่งด้านล่างแทน

  v.addEventListener("playing", show);
  v.addEventListener("error", hide);
  v.querySelector("source")?.addEventListener("error", hide);

  const tryPlay = v.play();
  if(tryPlay && tryPlay.catch) tryPlay.catch(()=>hide());

  setTimeout(()=>{ if(v.paused || v.readyState < 3) hide(); }, 4000);
});
...
(function(){
  ...
  function resize(){
    ...
    const count = Math.min(24, Math.floor(w/44));   // ลดจำนวนให้เบาบาง
  ...
  function newParticle(){
    return {
      ...
      a: Math.random()*0.35 + 0.12,          // ความสว่าง (เบาลง กลมกลืนวิดีโอ)
...
(function(){
  ...
  let timer = null;
  const start = ()=>{ if(!timer) timer = setInterval(dropPetal, 2600); };
...
(function(){
  ...
  let timer = null;
  const start = ()=>{ if(!timer){ flock(); timer = setInterval(flock, 20000); } };
  const stop  = ()=>{ clearInterval(timer); timer = null; };
  const io = new IntersectionObserver(es=>es.forEach(e=> e.isIntersecting ? start() : stop() ),{threshold:0});
  io.observe(hero);
})();
...
(function(){
  ...
  document.addEventListener("mousemove", e=>{
    ...
    if(now - last < 70) return;          // ลดความถี่ให้เบา
    last = now;
...
/* ═══ สะพานเชื่อมสำหรับหน้ารายละเอียด (place.js) ═══ */
window.state = state;
...
/* พูดข้อความด้วยเสียง (ใช้ Azure ก่อน ถ้าไม่ได้ใช้เบราว์เซอร์) — reuse ตัวเดิม */
window.narrateText = async function(text, btn){
  if(typeof stopNarration === "function") stopNarration();
  try{ await speakAzure(text); }
  catch(e){ speakBrowser(text); }
};

/* ═══ เปลี่ยนหน้าแบบ fade เนียน ═══ */
(function(){
  if(window.matchMedia("(prefers-reduced-motion:reduce)").matches) return;
  /* เฟดเข้าเมื่อโหลดหน้า */
  document.documentElement.classList.add("page-ready");

  /* เฟดออกก่อนเปลี่ยนหน้า (เฉพาะลิงก์ภายในเว็บ) */
  document.addEventListener("click", e=>{
    const a = e.target.closest("a");
    if(!a) return;
    const href = a.getAttribute("href") || "";
    /* ข้ามลิงก์นอก, สมอ, target ใหม่, ดาวน์โหลด, เมล/โทร */
    if(!href || href.startsWith("#") || href.startsWith("http") ||
       a.target === "_blank" || href.startsWith("mailto") || href.startsWith("tel") ||
...
/* ═══ เส้นใต้หัวข้อ section วิ่งเข้าเมื่อเลื่อนมาเห็น ═══ */
(function(){
  const heads = document.querySelectorAll(".section-head h2, .section-head .eyebrow");
  if(!heads.length) return;
  const io = new IntersectionObserver(es=>es.forEach(e=>{
    if(e.isIntersecting){ e.target.classList.add("head-lit"); io.unobserve(e.target); }
  }),{threshold:.5});
  document.querySelectorAll(".section-head h2").forEach(h=>{
    h.classList.add("head-underline");
    io.observe(h);
  });
})();
